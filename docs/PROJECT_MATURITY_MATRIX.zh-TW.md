...
| Renderer / Unreal / Simulation | `contract_only` | 已有 renderer/simulation/unreal bridge contract、Visual/Skin manifest reference contract、compact projection contract、ready-event factory 與 bounded event-log context。 | Unreal 不做 real I/O；simulation backend 是 `contract_only`；Visual/Skin registry 不讀 renderer payload，ready-event context 尚未實際寫入 runtime event log，不能說已實作引擎或 skin builder。 |
