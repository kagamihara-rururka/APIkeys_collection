...
| Renderer / Unreal / Simulation | `contract_only` | 已有 renderer/simulation/unreal bridge contract、Visual/Skin manifest reference contract 與 compact projection contract。 | Unreal 不做 real I/O；simulation backend 是 `contract_only`；Visual/Skin registry 不讀 renderer payload，不能說已實作引擎或 skin builder。 |
