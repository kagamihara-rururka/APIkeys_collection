...
| Renderer / Unreal / Simulation | `contract_only` | 已有 renderer/simulation/unreal bridge contract、Visual/Skin manifest reference contract、compact projection contract、ready-event factory、bounded event-log context、event writer、registry-entry writer helper、lifecycle display profile、summary status profiles、registry persistence schema contract、row projection contract、SQLite DDL dry-run preview、owned test-only table creation helper、owned test-only write/read/list helpers、owned test-only rollback/drop preview 與 owned test-only read-side summary helper。 | Unreal 不做 real I/O；simulation backend 是 `contract_only`；Visual/Skin registry 不讀 renderer payload，persistence schema / row projection / DDL / rollback preview / read-side summary 不寫使用者 DB，owned table/write/read/list/drop-preview/summary helper 只可用於明確 opt-in test DB，尚未接正式 user DB repository persistence 或自動 lifecycle emission，不能說已實作引擎或 skin builder。 |
