...
| Renderer / Unreal / Simulation | `contract_only` | 已有 renderer/simulation/unreal bridge contract、Visual/Skin manifest reference contract、compact projection contract、ready-event factory、bounded event-log context、event writer、registry-entry writer helper、persisted-entry explicit event writer、lifecycle display profile、summary status profiles、registry persistence schema contract、row projection contract、SQLite DDL dry-run preview、owned test-only table creation helper、owned test-only write/read/list helpers、owned test-only rollback/drop preview、owned test-only read-side summary helper、debug/agent summary CLI JSON 與 explicit ready-event CLI JSON。 | Unreal 不做 real I/O；simulation backend 是 `contract_only`；Visual/Skin registry 不讀 renderer payload，persistence schema / row projection / DDL / rollback preview / read-side summary 不寫使用者 DB，owned table/write/read/list/drop-preview/summary/event helper 與 summary/event CLI 只可用於明確 opt-in test DB，ordinary write/upsert 不會自動發 lifecycle event，尚未接正式 user DB repository persistence、UI integration 或自動 lifecycle emission，不能說已實作引擎或 skin builder。 |
