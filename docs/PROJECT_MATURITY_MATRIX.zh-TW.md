...
| Renderer / Unreal / Simulation | `contract_only` | 已有 renderer/simulation/unreal bridge contract、Visual/Skin manifest reference contract、compact projection contract、ready-event factory、bounded event-log context、event writer、registry-entry writer helper 與 lifecycle display profile。 | Unreal 不做 real I/O；simulation backend 是 `contract_only`；Visual/Skin registry 不讀 renderer payload，writer 尚未接 registry persistence 或自動 lifecycle emission，不能說已實作引擎或 skin builder。 |
