...
- Verification for this docs-only refresh: `tests.test_core_json_diagnostics_catalog -v` PASS (5 tests), OpenSpec validate PASS (3 specs), latest CI `26872795697` PASS.
- Boundary: docs/evidence alignment only. No product behavior change, no readiness promotion, no lifecycle/status schema change, no cross-repo integration, no renderer/compressor touch.
...
- Scope: Core readiness evidence only. No product behavior change, no lifecycle/status schema change, no readiness promotion, no cross-repo integration, no renderer/compressor touch.
...
- The first full smoke attempt exposed fixed temp DB coupling; the test now gives each diagnostic flag its own temporary directory and SQLite path.
- Scope: test-hardening only. No production code change, no CLI behavior change, no JSON payload change, no lifecycle/status schema change, no readiness promotion, no cross-repo integration.
- Verification: focused catalog tests PASS (5 tests); related Core diagnostic / CLI JSON tests PASS (11 tests); pre-push smoke PASS in `state/logs/pre_push_smoke_20260603_150015.log` (1152 tests, 4 skipped, MVP demo `download_import_completed`, `row_count=3`). Remote CI evidence is tracked in GitHub Actions / final checkpoint report.
...
- 邊界：這是 readiness evidence aggregation / import-boundary cleanup，不新增 review queue schema、不寫 queue record、不定義 resolution statuses、不改 lifecycle schema/status、不把 review-required promoted 成 ready、不 import 下游 repo、不讀 `.npz` / renderer payload。
- 已驗證：focused tests `tests.test_core_readiness_report tests.test_core_review_queue_readiness_report -v` 通過 12 tests；broader related tests `tests.test_core_readiness_report tests.test_core_review_queue_readiness_report tests.test_core_review_required_report tests.test_cli_flags tests.test_cli_json -v` 通過 18 tests；完整 smoke `state\logs\pre_push_smoke_20260603_052402.log` 通過 1143 tests / 4 skipped，MVP demo `download_import_completed` / `row_count=3`；code checkpoint `feb183a` 已由 GitHub Actions run `26849126467` 驗證通過 Ubuntu、Windows 與 real DB smoke。
- 下一個安全動作：若繼續 review queue，只能做 OpenSpec / dry-run schema preview / owned-test DB helper；正式 schema、resolution statuses、promotion policy 或 cross-repo review contract 都要先送 `o_1`。

## 2026-06-03 05:16 +08:00 Review item identity contract draft
- 本輪擴充 `api_launcher/core_review_queue_readiness_report.py` 與 `tests/test_core_review_queue_readiness_report.py`：`--core-review-queue-readiness-json` 現在會在 `existing_evidence.review_item_identity_contract_draft` 暴露 `core_review_item_identity_contract_draft.v1`，定義 review-required item 的最小穩定 identity shape：`review_item_id`、`source_surface`、`source_reference`、`review_bucket`、`next_action` 等欄位。
- 邊界：這是 contract-only identity draft，不新增 review queue schema、不寫 review queue record、不定義 resolution statuses、不改 lifecycle schema/status、不把 review-required promoted 成 ready、不 import 下游 repo、不讀 `.npz` / renderer payload；missing evidence 已從 `stable_review_item_identity_not_defined` 收斂為 `stable_review_item_identity_not_persisted`。
- 已驗證：focused tests `tests.test_core_review_queue_readiness_report -v` 通過 6 tests；broader related tests `tests.test_core_review_queue_readiness_report tests.test_core_review_required_report tests.test_core_readiness_report tests.test_cli_flags tests.test_cli_json -v` 通過 18 tests；完整 smoke `state\logs\pre_push_smoke_20260603_051050.log` 通過 1143 tests / 4 skipped，MVP demo `download_import_completed` / `row_count=3`；code checkpoint `9327669` 已由 GitHub Actions run `26848479833` 驗證通過 Ubuntu、Windows 與 real DB smoke。
- 下一個安全動作：若繼續 review queue，只能做 OpenSpec / dry-run schema preview / owned-test DB helper；正式 schema、resolution statuses、promotion policy 或 cross-repo review contract 都要先送 `o_1`。
...
- Broader related tests `tests.test_core_bounded_scheduler_plan_report tests.test_core_job_status_report tests.test_core_readiness_report tests.test_project_maturity tests.test_tk_background_jobs tests.test_visual_asset_contracts tests.test_visual_asset_event_logging tests.test_cli_flags tests.test_cli_json -v` 通過 71 tests；完整 smoke `state\logs\pre_push_smoke_20260603_043016.log` 通過 1140 tests / 4 skipped，MVP demo `download_import_completed` / `row_count=3`；code checkpoint `711b016` 已由 GitHub Actions run `26846446579` 驗證通過 Ubuntu、Windows 與 real DB smoke。
...
- Broader related tests `tests.test_core_bounded_scheduler_plan_report tests.test_core_job_status_report tests.test_core_readiness_report tests.test_project_maturity tests.test_tk_background_jobs tests.test_visual_asset_contracts tests.test_visual_asset_event_logging tests.test_cli_flags tests.test_cli_json -v` 通過 69 tests；完整 smoke `state\logs\pre_push_smoke_20260603_041537.log` 通過 1138 tests / 4 skipped，MVP demo `download_import_completed` / `row_count=3`；code checkpoint `463de67` 已由 GitHub Actions run `26845690026` 驗證通過 Ubuntu、Windows 與 real DB smoke。
...
- 完整 smoke `state\logs\pre_push_smoke_20260603_035830.log` 通過 1136 tests / 4 skipped，MVP demo `download_import_completed` / `row_count=3`；code checkpoint `e936080` 已由 GitHub Actions run `26844820055` 驗證通過 Ubuntu、Windows 與 real DB smoke。
...
- 邊界：這是 report-only / contract-coverage slice，不實作 scheduler runtime、不新增 queue migration、不改 lifecycle schema/status、不啟用 auto lifecycle event、不 import 下游 repo、不讀 `.npz` / renderer payload。
- 已驗證：`py_compile` OK；focused tests `tests.test_core_bounded_scheduler_plan_report tests.test_core_job_status_report tests.test_cli_flags tests.test_cli_json -v` 通過 17 tests；broader related tests `tests.test_core_bounded_scheduler_plan_report tests.test_core_job_status_report tests.test_core_readiness_report tests.test_project_maturity tests.test_tk_background_jobs tests.test_visual_asset_contracts tests.test_cli_flags tests.test_cli_json -v` 通過 58 tests；JSON stdout 可由 downstream `json.load(sys.stdin)` 解析，輸出 `schema_version=scheduler_lane_contract_coverage.v1`、`status=partial`、`missing_policy_facets=timeout_policy,retry_policy,cancellation_policy,review_policy`、`treats_tk_policy_registry_as_full_scheduler=false`。
- CI evidence：code commit `75923c0 feat(core): report scheduler lane contract coverage` 已 push；GitHub Actions manual run `26843841155` 通過 Ubuntu、Windows 與 real DB smoke。
- OpenSpec：`bounded-scheduler-core-contract` task 3.1 已完成。下一個安全動作可做 3.2 cancellation/retry/timeout/review-required/blocked-job next-action payload tests；若要正式 queue migration、runtime scheduler、automatic lifecycle event、cross-process SQLite lock、或 asyncio migration，先送 `o_1`。

## 2026-06-03 03:17 +08:00 Core scheduler owned-test queue table helper
- 本輪擴充 `api_launcher/core_scheduler_persistence_contract.py`：新增 `create_scheduler_queue_table_for_owned_test_database()` 與 `scheduler_queue_owned_test_table_helper_contract()`，讓 scheduler queue DDL 只能在明確 `allow_owned_test_database=True` 的 owned test SQLite DB materialize。
- `--core-bounded-scheduler-plan-json` 現在在 `existing_evidence.scheduler_owned_test_table_helper` 暴露 helper contract；missing evidence 仍保守維持 `durable_job_queue_persistence_not_promoted_beyond_owned_test`，不宣稱正式 durable queue persistence。
- 邊界：這是 owned-test helper，不是 user DB migration、不寫 job rows、不實作 scheduler runtime、不改 lifecycle schema/status、不啟用 auto lifecycle event、不 import 下游 repo、不讀 `.npz` / renderer payload。既有 unowned SQLite DB 會拒絕，未開 gate 不會建檔。
- 已驗證：`py_compile` OK；focused tests `tests.test_core_bounded_scheduler_plan_report tests.test_core_job_status_report tests.test_cli_flags tests.test_cli_json -v` 通過 16 tests；broader related tests `tests.test_core_bounded_scheduler_plan_report tests.test_core_job_status_report tests.test_core_readiness_report tests.test_project_maturity tests.test_tk_background_jobs tests.test_visual_asset_contracts tests.test_cli_flags tests.test_cli_json -v` 通過 57 tests；JSON stdout 可由 downstream `json.load(sys.stdin)` 解析，輸出 `scope=owned_test_database_only`、`missing_evidence[1]=durable_job_queue_persistence_not_promoted_beyond_owned_test`、`user_database_write_allowed=false`；`git diff --check` OK；完整 smoke `state\logs\pre_push_smoke_20260603_031841.log` 通過 1133 tests / 4 skipped，MVP demo `download_import_completed` / `row_count=3`。
- CI evidence：code commit `cf62c31 feat(core): add owned scheduler queue test helper` 已 push；GitHub Actions manual run `26842825576` 通過 Ubuntu、Windows 與 real DB smoke。
- 下一個安全動作：若繼續此 OpenSpec，可做 status/policy bridge 比對報表或 cancellation/retry/timeout next-action payload；若要正式 queue migration、runtime scheduler、automatic lifecycle event、cross-process SQLite lock、或 asyncio migration，先送 `o_1`。
