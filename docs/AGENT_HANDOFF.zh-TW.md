...
- 已驗證：focused `py_compile` OK；`py -3 -B -m unittest tests.test_core_lifecycle_audit_report tests.test_core_job_status_report tests.test_core_readiness_report tests.test_cli_flags tests.test_cli_json -v` 通過 15 tests；實跑 `--core-lifecycle-audit-json` 可由 downstream `json.load(sys.stdin)` 解析，輸出 `schema_version=core_lifecycle_audit_report.v1`、`status=partial`、`status_count=7`。
