...
- 下一個安全動作：跑完整 smoke 後提交；若繼續此 OpenSpec，可做 status/policy bridge 比對報表或 cancellation/retry/timeout next-action payload；若要正式 queue migration、runtime scheduler、automatic lifecycle event、cross-process SQLite lock、或 asyncio migration，先送 `o_1`。
