...
| `VISUAL_SKIN_ASSET_CONTRACT.zh-TW.md` | RRKAL Core 對未來 RendererSkinAsset 的控制面契約，說明 build request、skin reference、registry entry、ready event、event log writer、lifecycle status 與禁止碰 renderer payload 的邊界。 | 變更 `api_launcher/visual_asset_contracts.py`、`api_launcher/visual_asset_event_logging.py`、project maturity renderer row、Visual/Skin registry lifecycle、跨 displaytools / compressor manifest reference 時更新。 |
