...
| 要快速引用 Core evidence / pattern index | `CORE_EVIDENCE_AND_PATTERN_INDEX.zh-TW.md` -> `CORE_READINESS_EVIDENCE_PACKET.zh-TW.md` -> `CORE_CONTROL_PLANE_RESPONSIBILITY_AUDIT.zh-TW.md` -> `CAPABILITY_ADDRESSING_PATTERN.zh-TW.md` | 給 `n_1` / `o_1` / Owner 快速確認目前 Core gate、evidence source、Notion wording boundary 與 capability addressing pattern；不得寫成 integration authorization。 |
...
| 要理解 capability address / mask / decorator registry 模式 | `CAPABILITY_ADDRESSING_PATTERN.zh-TW.md` -> `DATASET_DISCOVERY_NOTES.zh-TW.md` -> `api_launcher/crawlers/registry.py` | 先看 4-bit capability addressing 的限制：CapabilityCode 是索引，不是完整真相；`CrawlerSpec` 仍是語意來源。 |
...
| `CORE_EVIDENCE_AND_PATTERN_INDEX.zh-TW.md` | Core evidence / pattern 快速索引，彙整 readiness packet、control-plane responsibility audit、capability addressing ADR、CI / CLI JSON / OpenSpec evidence 與 Notion wording boundary。 | Core evidence packet、control-plane audit、capability addressing ADR、CI run、Core JSON diagnostics、OpenSpec validate 或 `n_1` summary boundary 改變時更新；不得寫成 integration authorization。 |
...
| `CAPABILITY_ADDRESSING_PATTERN.zh-TW.md` | RRKAL Core capability addressing ADR，整理目前 crawler registry 的 4-bit capability code、CIDR-style mask、decorator registration 與 `CrawlerSpec` semantic boundary。 | Crawler registry capability dimensions、mask semantics、decorator safety boundary 或未來 adoption policy 改變時更新；不得寫成 universal capability framework 或 integration authorization。 |
